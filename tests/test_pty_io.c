...
void setUp(void) {
  ...
  TEST_ASSERT_EQUAL_INT_MESSAGE(
			  ...
			  "openpty() failed - is a pty available in this environment?");
  ...
  cfmakeraw(&raw);
  TEST_ASSERT_EQUAL_INT(0, tcsetattr(slave_fd, TCSANOW, &raw));
...
static void type_bytes(const unsigned char *bytes, size_t len) {
...
void test_plain_char_is_available_and_read_correctly(void) {
...
void test_escape_sequence_byte_is_not_lost_by_char_available_check(void) {
 unsigned char up_arrow[] = { 27, '[', 'A' }; /* ESC [ A = Up arrow */
...
void test_repeated_char_available_checks_are_idempotent(void) {
...
int main(void) {
  ...
  RUN_TEST(test_escape_sequence_byte_is_not_lost_by_char_available_check);
