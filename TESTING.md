...
| Decoder logic | `tests/test_decoder.c` | Any (mocked) | The Linux escape-sequence state machine (`cpnbi__decode_event`), tested via mocked `next_byte`/`more_available` functions - no terminal needed. Covers arrows, F-keys, Home/End/Insert/Delete/PageUp/PageDown, all modifier combinations, lone Esc, and unrecognized sequences. |
...
- Windows-side logic (`cpnbi__process_event`) is structurally testable the same way as the decoder (it already takes a plain `INPUT_RECORD*` with no I/O inside it), but no Windows test suite exists yet.
